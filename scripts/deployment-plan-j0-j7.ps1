...
function Start-PreFlightChecks {
    ...
    try {
        ...
        if (-not $DryRun) {
            Write-PhaseLog "Lancement test-security-plan.ps1..." "INFO"
        ...
        if (-not $DryRun) {
            Write-PhaseLog "Lancement create-release-prod.ps1..." "INFO"
            if (Test-Path ".\scripts\create-release-prod-clean.ps1") {
                ...
                if ($LASTEXITCODE -ne 0) { throw "Build production echoue" }
            } else {
        ...
        $expectedFiles = @(
            "releases\$Version\USB-Video-Vault-Setup.exe",
            "releases\$Version\USB-Video-Vault-Setup.msi",
        ...
        foreach ($file in $expectedFiles) {
            if (Test-Path $file) {
                ...
                Write-PhaseLog "WARN Artefact manquant: $file" "ERROR"
                if (-not $DryRun) { throw "Artefact manquant: $file" }
            }
        }
...
function Start-Ring0Preparation {
    ...
    try {
        ...
        foreach ($machine in $deploymentConfig.Ring0Machines) {
            ...
            if (-not $DryRun) {
                try {
                    $licenseOutput = & node .\scripts\make-license.mjs $machine.Fingerprint $machine.UsbSerial 2>&1
                    if ($LASTEXITCODE -eq 0) {
                        $licenseFile = "deliveries\$($machine.Name)-license.bin"
                        $sourceLicense = "vault-real\.vault\license.bin"
                        if (Test-Path $sourceLicense) {
                            Copy-Item $sourceLicense $licenseFile -Force
                        ...
                        Write-PhaseLog "ERROR Erreur generation licence: $licenseOutput" "ERROR"
                    }
                } catch {
...
function Start-Ring0Deployment {
    ...
    try {
        ...
        foreach ($machine in $deploymentConfig.Ring0Machines) {
            ...
            $licenseFile = "deliveries\$($machine.Name)-license.bin"
            $installerPath = "releases\$Version\USB-Video-Vault-Setup.exe"

            if (-not $DryRun) {
                try {
                    $installOutput = & .\scripts\ring0-install.ps1 `
                    ...
                    if ($LASTEXITCODE -eq 0) {
                        Write-PhaseLog "OK Installation reussie: $($machine.Name)" "SUCCESS"
                        $result = @{
                        ...
                        Write-PhaseLog "ERROR Installation echouee: $($machine.Name)" "ERROR"
                        $result = @{
                            Machine = $machine.Name
                            Status = "FAILED"
                            Timestamp = Get-Date
...
function Test-Ring0GoNoGo {
    ...
    try {
        ...
        if (-not $DryRun) {
            ...
            # (Logique d'analyse a brancher si besoin)
                ...
                if (Test-Path $licenseFile) {
                    try {
                        ...
                        if ($LASTEXITCODE -ne 0) {
                            ...
                            $vo = ($verifyOutput | Out-String).Trim()
                            if ($vo.Length -gt 300) { $vo = $vo.Substring(0,300) + "..." }
                            Write-PhaseLog "ERROR Licence invalide pour $($machine.Name) : $vo" "ERROR"
                        } else {
                            Write-PhaseLog "OK Licence OK ($($machine.Name))" "SUCCESS"
                        }
                    } catch {
                        $goNoGoResults.LicenseValidation = $false
                        Write-PhaseLog "ERROR Erreur verification licence ($($machine.Name)) : $($_.Exception.Message)" "ERROR"
                    }
                } else {
                    $goNoGoResults.LicenseValidation = $false
                    Write-PhaseLog "ERROR Licence absente pour $($machine.Name)" "ERROR"
                }
            }
        } else {
...
function Start-DeploymentPlan {
    ...
    try {
        ...
        foreach ($phaseResult in $phaseResults) {
            ...
            $statusLevel = if ($phaseResult.Success) { "SUCCESS" } else { "ERROR" }
            Write-PhaseLog "$($phaseResult.Phase): $status" $statusLevel
